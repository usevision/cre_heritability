---
title: "plot_heatmap_sldsc_metaclusters"
author: "Kate Weaver"
date: "2022-12-13"
...
fix our labels to be prettier/match the other figure
...
## Focusing on male .... make those changes
...
gf <- ggplot(femMatWithLabel, aes(y=factor(jmc, levels = rev(c( "1","4","12","9","8","10","2","5","3","15","7","11","14","6","13"))) , x=label)) + geom_tile(aes(fill=factor(0)), show.legend = FALSE) + geom_point(aes(colour = log2(color), size = -log10(size))) + theme_bw() + ylab("JmC") + xlab("") + scale_fill_manual(values = c("transparent")) + theme(axis.text.x = element_text(angle = 45,hjust=1, color=tick_colors_fem, size=8)) + labs(size = "-log10(Adjusted p-value)", colour = "log2(Enrichment)") + theme(plot.margin = margin(1.5, 1.75, 1, 1.75, "cm")) + ggtitle("JmC Trait Enrichments (Female)") + theme(panel.grid = element_line(color="black", size=0.1))
gf
ggsave("~/mccoyLab/VISION/sldsc/jmc_female_angle.pdf", plot = gf)
```

```{r}
gf2 <- ggplot(femMatWithLabel, aes(y=factor(jmc, levels = rev(c( "1","4","12","9","8","10","2","5","3","15","7","11","14","6","13"))) , x=label)) + geom_tile(aes(fill=factor(0)), show.legend = FALSE) + geom_point(aes(colour = log2(color), size = -log10(size))) + theme_bw() + ylab("JmC") + xlab("") + scale_fill_manual(values = c("transparent")) + theme(axis.text.x = element_text(angle = 90, vjust=0.5,hjust=1, color=tick_colors_fem, size=8)) + labs(size = "-log10(Adjusted p-value)", colour = "log2(Enrichment)") + theme(plot.margin = margin(0.5, 0, 0, 0.25, "cm")) + ggtitle("JmC Trait Enrichments (Female)") + theme(panel.grid = element_line(color="black", size=0.1))
gf2
ggsave("~/mccoyLab/VISION/sldsc/jmc_female_straight.pdf", plot = gf2)
```
...
gm <- ggplot(maleMatWithLabel, aes(y=factor(jmc, levels = rev(c( "1","4","12","9","8","10","2","5","3","15","7","11","14","6","13"))) , x=label)) + geom_tile(aes(fill=factor(0)), show.legend = FALSE) + geom_point(aes(colour = log2(color), size = -log10(size))) + theme_bw() + ylab("JmC") + xlab("") + scale_fill_manual(values = c("transparent")) + theme(axis.text.x = element_text(angle = 45,hjust=1, color=tick_colors_male, size=8)) + labs(size = "-log10(Adjusted p-value)", colour = "log2(Enrichment)") + theme(plot.margin = margin(1, 2, 1, 1.5, "cm")) + ggtitle("JmC Trait Enrichments (Male)") + theme(panel.grid = element_line(color="black", size=0.1))
gm
ggsave("~/mccoyLab/VISION/sldsc/jmc_male_angle.pdf", plot = gm)
```

```{r}
gm2 <- ggplot(maleMatWithLabel, aes(y=factor(jmc, levels = rev(c( "1","4","12","9","8","10","2","5","3","15","7","11","14","6","13"))) , x=label)) + geom_tile(aes(fill=factor(0)), show.legend = FALSE) + geom_point(aes(colour = log2(color), size = -log10(size))) + theme_bw() + ylab("JmC") + xlab("") + scale_fill_manual(values = c("transparent")) + theme(axis.text.x = element_text(angle = 90,vjust=0.5, hjust=1, color=tick_colors_male, size=8)) + labs(size = "-log10(Adjusted p-value)", colour = "log2(Enrichment)") + theme(plot.margin = margin(0.75, 0, 0, 0.25, "cm")) + ggtitle("JmC Trait Enrichments (Male)") + theme(panel.grid = element_line(color="black", size=0.1))
gm2
ggsave("~/mccoyLab/VISION/sldsc/jmc_male_straight.pdf", plot = gm2)
```
